...
projectsRouter.post("/", async (req, res) => {
  ...
  if (Array.isArray(body)) {
    if (!canAdmin(req.user!.role)) {
      res.status(403).json({ error: "Only Administrators can run a bulk import." });
      return;
    }
...
projectsRouter.post("/import", async (req, res) => {
  if (!canAdmin(req.user!.role)) {
    res.status(403).json({ error: "Only Administrators can import projects." });
    return;
  }
  const rows = Array.isArray(req.body?.rows) ? req.body.rows : [];
  if (!rows.length || rows.length > 500) {
    res.status(400).json({ error: "Provide between 1 and 500 project rows." });
    return;
  }
  const errors: Array<{ row: number; error: string }> = [];
  const codes = new Set<string>();
  const valid = rows.map((row: Record<string, unknown>, index: number) => {
    const projectName = String(row.projectName || row.title || "").trim();
    const projectCode = String(row.projectCode || "").trim() || null;
    if (!projectName) errors.push({ row: index + 1, error: "projectName is required" });
    if (projectCode && codes.has(projectCode)) errors.push({ row: index + 1, error: "Duplicate projectCode in import" });
    if (projectCode) codes.add(projectCode);
    return { projectName, projectCode, district: String(row.district || "Punjab"), year: String(row.year || "2025-26"), mineral: String(row.mineral || "Sand"), rivers: String(row.rivers || "Not specified"), status: parseStatus(row.status) };
  });
  if (errors.length || req.body?.dryRun) {
    res.json({ valid: errors.length === 0, errors, preview: valid });
    return;
  }
  const created = await prisma.$transaction(valid.map((row) => prisma.project.create({ data: { ...row, title: row.projectName, createdBy: req.user!.id } })));
  res.status(201).json(jsonSafe({ imported: created.length, projects: created.map(toProjectDto) }));
});
